...
import tlcMain from "../../../assets/myprojectsection/TLC/tl1.png";
import tlcQuickUi from "../../../assets/quickui/qtlc.png";
import tlc1 from "../../../assets/myprojectsection/TLC/tl1.png";
import tlc2 from "../../../assets/myprojectsection/TLC/tl2.png";
import tlc3 from "../../../assets/myprojectsection/TLC/tl3.png";
import tlc4 from "../../../assets/myprojectsection/TLC/tl4.png";
import tlc5 from "../../../assets/myprojectsection/TLC/tl5.png";

export default function TlcEngineProjectPage() {
	const galleryImages = [tlc1, tlc2, tlc3, tlc4, tlc5];
	const titleText = "TLC engine";
	...
	return (
		<main className="min-h-screen bg-[#d7d9e1]">
			<NavbarProjects backgroundColor="#006B5F" backHref="/allprojects" />

			<div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 md:px-10">
				<h1 ref={titleRef} className="mb-12 text-center text-4xl font-semibold text-[#006B5F] md:mb-16 md:text-5xl">
				...
				<section className="grid gap-10 md:grid-cols-[1fr_1.4fr] md:gap-12">
					<div>
						<div ref={mainPreviewRef} className={`${mainPreviewVisible ? "slide-in-left-right" : "opacity-0"} group mx-auto w-full max-w-sm md:max-w-md`}>
							<Image
								src={tlcMain}
								alt="TLC engine dashboard preview"
								width={520}
								height={320}
						...
						<div ref={projectLinkRef} className={`${projectLinkVisible ? "slide-in-bottom-up" : "opacity-0"} mt-5 text-center`}>
							<h2 className="text-xl font-semibold text-[#006B5F]">Project link</h2>
							<Link
								href="#"
					...
					<div className="space-y-8">
						...
						>
							<h2 className="text-2xl font-semibold text-[#006B5F] md:text-2xl">Description</h2>
							<p className="mt-2 text-justify text-lg leading-relaxed text-[#1b3553]">
								TLC Engine is a scalable multi-tenant platform with role-based access and
								separate environments per organization. I handled backend development and
								contributed to frontend features to ensure a stable and practical product.
							</p>
						</div>
						...
						>
							<h2 className="text-2xl font-semibold text-[#006B5F] md:text-2xl">Role</h2>
							<p className="mt-2 text-justify text-lg leading-relaxed text-[#1b3553]">
								I built backend services, designed data flow and access control, and also
								supported frontend integration to keep the system efficient and reliable.
							</p>
						</div>
						...
						>
							<h2 className="text-2xl font-semibold text-[#006B5F] md:text-2xl">Challenges and Solution</h2>
							<p className="mt-2 text-justify text-lg leading-relaxed text-[#1b3553]">
								A major challenge was supporting multiple tenants with isolated data and
								stable performance. I optimized backend architecture and improved data
								handling to keep each environment responsive and secure.
							</p>
						</div>
					</div>
				</section>
				...
				<section className="mt-16 md:mt-20">
					<h2 className="text-center text-2xl font-semibold text-[#006B5F] md:text-xl">Quick UI</h2>
					<div className="mt-5 flex justify-center">
						<button
							...
							aria-label="Open TLC engine gallery"
						>
							<Image
								src={tlcQuickUi}
								alt="TLC engine quick UI preview"
								width={760}
								height={420}
			...
			{isGalleryOpen && (
				...
				>
					<div
						className="relative w-full max-w-5xl rounded-3xl border border-white/55 bg-linear-to-br from-[#e8f5f2]/88 via-[#d8efe8]/86 to-[#c9e7de]/84 p-4 shadow-[0_20px_60px_rgba(27,53,83,0.22)] backdrop-blur-md md:p-6"
						onClick={(event) => event.stopPropagation()}
					>
						<button
							type="button"
							onClick={() => setIsGalleryOpen(false)}
							className="absolute right-3 top-3 rounded-md border border-[#006B5F]/30 bg-white/75 px-2.5 py-1 text-sm font-semibold text-[#1b3553] transition-colors hover:bg-white"
						>
							Close
						</button>
						...
						<div className="relative mx-auto mt-6 w-full max-w-4xl select-none overflow-hidden rounded-2xl border border-white/70 bg-white/45 aspect-video">
							<div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-[22%] overflow-hidden rounded-l-2xl border-r border-white/40">
								<Image
									...
									alt={`TLC engine gallery image ${previousIndex + 1}`}
									width={1200}
									height={750}
									draggable={false}
							...
							<div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-[22%] overflow-hidden rounded-r-2xl border-l border-white/40">
								<Image
									...
									alt={`TLC engine gallery image ${nextIndex + 1}`}
									width={1200}
									height={750}
									draggable={false}
									className="h-full w-full translate-x-6 scale-[1.02] object-cover opacity-35 blur-[1px]"
							...
							>
								<Image
									...
									alt={`TLC engine gallery image ${galleryIndex + 1}`}
									width={1200}
									height={750}
									draggable={false}
							...
							<button
								...
								className="absolute left-3 top-1/2 z-30 -translate-y-1/2 rounded-full border border-[#006B5F]/35 bg-white/80 p-2.5 text-[#1b3553] transition-colors hover:bg-white"
							...
							<button
								type="button"
								onClick={showNext}
								className="absolute right-3 top-1/2 z-30 -translate-y-1/2 rounded-full border border-[#006B5F]/35 bg-white/80 p-2.5 text-[#1b3553] transition-colors hover:bg-white"
						...
						<div className="mt-4 flex items-center justify-center">
							<p className="rounded-full border border-[#006B5F]/35 bg-white/80 px-4 py-1.5 text-sm font-medium text-[#1b3553]">
